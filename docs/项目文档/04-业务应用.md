...
> 8. **流式链路改版（2026-09-13）**：三处 O(n²) 与一处丢数据，细节见 `docs/zrpc-migration/12-zrpc-implementation-and-usage.md` §6 与各 commit：
...
本项目的"对外公网"不是把整套服务搬到云上，而是 **FRP 内网穿透**：云端一台 2C2G 只做**入口转发**（frps + Nginx + Certbot），**完整的 AnswerMesh（含 semantic 语义检索/tokenizer/kvstore/MySQL/向量库）始终只跑在本地节点**。完整技术文档见 `docs/answermesh-frp-nat-traversal.md`。
...
2. 健康检查（**都用 HTTP，没有 `grpc_health_probe` 了**）：
   ...
   - `docs/grpc_health_probe-linux-amd64` 与 `ai-chat-service/Dockerfile:9-10` 里的同名二进制是**历史残留**，不要再用。
...
- **zrpc 量化对比（务必带限定语）**：迁移期做过单 unary 微基准（`docs/zrpc-migration/08-benchmark-filter.md`，同进程双栈 gRPC 50053 vs zrpc 50063，并发 8 × 5000 次）：**QPS 33,631 vs 26,605（+26%）**、avg 230µs vs 297µs、p99 **689µs vs 1542µs**。⚠ 该数字是**单盒 dev、单一 unary、首轮快照**，原文档自己也声明"不作为'必然更快'的定论"——引用时不能丢掉这个前提。
