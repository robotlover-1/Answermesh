...
> **更新(2026-09-04 · zrpc 迁移)**：backend↔chat-service 与 chat-service↔keywords-filter 的服务间调用已由 gRPC **迁移到自研 C zrpc v2**（原 gRPC 传输已删，观察期结束）。端口不变：filter 50053/50054、chat-service 50055，均走 zrpc。两条链路用同一业务方法提供，语义一致（含语义缓存 `source=cache`、断连取消）。实现与使用见 `third_party/zrpc/README.md` 与 `zrpc-go/`。

> **更新(2026-09-13 · 全文按代码复核)**：逐节核对当前代码后修正了一批过时描述，**读本文以本节为准**：
>
> 1. **服务间已无 gRPC**（zrpc v2 单栈）：方法名为 **`chat.completion` / `chat.completion_stream` / `filter.validate` / `filter.find_all`**（短点号名，非 gRPC 全名）；架构图/时序图/鉴权表都已改。**迁移期的 50063/50064 端口已不存在**（仅压在 `cmd/benchf` 里），filter 的 zrpc 端口就是 50053/50054。gRPC 相关代码（`grpc-go` 依赖、`.pb.go`、`interceptor/auth.go`、`grpc_health_probe`）均**为死代码/历史残留**，不在业务链路上。
> 2. **额度（quota）扣减逻辑已移除**：`ChatProcess` 不再做额度预检、不再 `DeductQuota`（该方法已是死代码），改为**只统计** `tokensUsed`（LLM）与 `tokensSaved`（缓存命中）；`users.quota` 字段与响应里的 `quota` 仍保留（前端展示用）。§5.3 已按此重写。
> 3. **健康检查不再用 `grpc_health_probe`**：filter 用 HTTP `:18081`/`:18082` 的 `/healthz` `/readyz`，service 用 HTTP `:8080` 的 `/healthz` `/readyz` `/metrics`（**端口硬编码**，与业务口 50055 分离），backend 用 `/api/health`（仅存活）+ `/api/readyz`（并行探 mysql/kvstore/tokenizer/service）；`docker/compose.yaml` 里已无 healthcheck 段，就绪判据由 `deploy/scripts/deploy-app.sh` 用 `curl /api/readyz` 承担。
> 4. **生产部署已从 `ai-chat-stack/`（Docker Swarm + Harbor 私有镜像）切到 `docker/compose.yaml`（build-from-source）+ `deploy/`（FRP）**：前者端口/模型（50051、`gpt-3.5-turbo`、`replicas: 2`）全是过期内容，仅存档。
> 5. **`frpc` 已纳入 `./start.sh` / `./stop.sh` 一键启停**（`start.sh:137-186`），不再是独立手工步骤；无 `deploy/app/.env` 或 `bin/frpc` 时自动跳过。
> 6. **品牌：2026-09-13 已全线更名 AnswerMesh**：前端标题与 PWA manifest 名、compose 项目名（`answermesh` / `answermesh-frpc` / `answermesh-edge`）、边缘 nginx conf 文件名（`answermesh.conf`，连同限流 zone `answermesh_api`）、frpc 代理名（`answermesh-web`）、仓库/文档全部统一；公网域名 `answermesh.xyz`（`ai-chat-web/index.html:10`）。**唯一没改的是组件名与库名**——module/目录仍是 `ai-chat-backend`/`ai-chat-service`/`ai-chat-web`，MySQL 库仍是 `ai_chat`：那是组件命名不是项目品牌。注意功能标识符改名**需要重新部署边缘/应用节点**（compose 项目名变了、nginx 挂载文件名变了，旧容器组要先 `down`）。
> 7. **模型**：线上为 `deepseek-v4-flash`（可配，请求级可覆盖）；`ai-chat-stack` 与 mock/tokenizer 里出现的 `gpt-3.5-turbo` 只是模型名映射表/历史配置，不代表线上模型。
> 8. **流式链路改版（2026-09-13）**：三处 O(n²) 与一处丢数据，细节见各 commit：
>    a) **backend 不再每帧发累积全文**（`chat.go`）：原先每帧 `json.Marshal(result)` 而 `result.Text` 是至今全文，单帧载荷随回答线性增长 → 整条流 O(n²)（3000 字回答约 1.4MB，只发增量约 240KB）。现**中间帧只发 `delta`，本段全文只在末帧给一次**；前端相应改为 `acc += delta` 累积（`index.vue` 两处）。
>    b) **删除每 15 个 chunk 的周期性 token 统计**：它用累积全文打两次同步 HTTP 到 tokenizer，调用次数与载荷都是 O(n²) 且阻塞在 `Recv` 循环里。现只在流结束时统计一次，数值相同。
>    c) **末帧补尾随 `\n`** 作记录终止符：前端 `createNdjsonReader` 只解析以 `\n` 结束的行，缺它这一帧永远留在 carry 里 → `tokensUsed`/`tokensSaved` 从未真正到达前端（老问题）。同时该读取器原先每个 progress 事件只解析「最后一行」，帧变小后会一次挤进好几帧、其余被静默丢弃（表现为回答内容错乱缺块），现已改为一次解析所有完整行。
>    d) **zrpc 客户端流事件队列（`evCh`，容量 128）由"满则丢弃"改为阻塞背压**：消费者（HTTP 写回浏览器）一慢就丢分片，表现为回答缺段；丢弃 STREAM_END 时前端永远转圈。现阻塞等待，靠 TCP 窗口把压力传导给上游（`zrpc-go/stream.go`，回归用例 `TestStreamSlowConsumerLosesNothing`）。
...
> 一句话：**kvstore 管临时（会话/多轮上下文/语义缓存），MySQL 管永久（用户记录 / 聊天历史）**。额度字段虽仍在 `users` 表里，但**扣减逻辑已于 2026-09 移除**（只统计 token，不再扣减）。
...
| 4 | **sensitive 敏感词** | Go | 50053 / zrpc | AC 自动机（第三方 `importcjj/sensitive`），命中即拦截回答；健康口 18081 |
| 5 | **keyword 关键词** | Go | 50054 / zrpc | 提取关键词（**仅供落库 `chat_records`**，不参与决策；rerank 已废弃）；健康口 18082 |
| 6 | **mock-openai-api** | Go | 8083 / HTTP | 模拟 OpenAI，无真实 key 时用 |
| 7 | **openai-api-proxy** | Go | 8084 / HTTP | 反向代理 + 鉴权 + 限流，集中管真 DeepSeek key（多 key 随机轮转） |
| 8 | **ai-chat-service** | Go | 50055 / zrpc(流式) | 核心编排：敏感词→语义缓存→关键词→上下文/token 预算→调模型→流式转发；健康口 8080 |
| 9 | **ai-chat-backend** | Go(Gin) | 7080 / HTTP | 网关：托管前端 + 免密登录(device_id) + 限流 + 鉴权 + token 统计 + NDJSON 流式（**无额度校验**） |
...
flowchart TB
    subgraph client["客户端"]
        A["浏览器 Vue3 前端（页面标题 AnswerMesh）· 本机 :7080 或 公网 https://<域名>"]
    end

    subgraph gateway["网关层"]
        B["ai-chat-backend (Gin HTTP) :7080<br/>托管前端 + 免密登录(device_id) + 限流10/s<br/>+ 鉴权 + token 统计 + NDJSON 流式返回"]
    end

    subgraph core["核心编排"]
        C["ai-chat-service zrpc :50055<br/>敏感词→语义缓存→关键词→上下文/token预算<br/>→调大模型→流式分片转发"]
    end

    subgraph filter["过滤层"]
        D["sensitive 敏感词 zrpc :50053"]
        E["keyword 关键词 zrpc :50054"]
    end
    ...
    subgraph data["数据层"]
        ...
        M["MySQL :3306<br/>users(quota 仅展示) / chat_records"]
    end

    subgraph llm["大模型层"]
        P["openai-api-proxy :8084<br/>鉴权+限流+集中管真key(多key轮转)"]
        RL["DeepSeek API<br/>deepseek-v4-flash"]
        MK["mock-openai-api :8083"]
    end

    A -- "HTTP /api/chat-process" --> B
    B -- "zrpc 流式 chat.completion_stream" --> C
    B -- "SETEX/GET 会话(device_id)" --> KV
    C -- "zrpc filter.validate 敏感词" --> D
    C -- "zrpc filter.find_all 关键词" --> E
    ...
    B -- "写 chat_records + token 统计(不再扣额度)" --> M
...
启动顺序（`lib.sh:12-22`）：kvstore → tokenizer → semantic → sensitive → keyword → mock → proxy → service → backend（依赖方在后）。
...
| 4 | **sensitive** | Go `keywords-filter`（敏感词典） | 50053 / zrpc | service（`filter.validate`） |
| 5 | **keyword** | Go `keywords-filter`（关键词词典） | 50054 / zrpc | service（`filter.find_all`） |
| 6 | **mock** | Go `mock-openai-api` | 8083 / HTTP | service **可选**（base_url 指向它时；默认不在链路上） |
| 7 | **proxy** | Go `openai-api-proxy` | 8084 / HTTP | service（POST /v1/chat/completions） |
| 8 | **service** | Go `ai-chat-service` | 50055 / zrpc 流式 | backend（`chat.completion_stream`） |
| 9 | **backend** | Go `ai-chat-backend` | 7080 / HTTP | 浏览器前端 |

> sensitive 与 keyword 是**同一个二进制 `keywords-filter`**，靠 `--config=dev.config.yaml` / `dev.kw.config.yaml` + 不同词典（`--dict=dict.txt` / `keyword-dict.txt`）各起一个实例（所以是 9 进程 / 8 个二进制）。端口表见 `lib.sh:12-22`。

**通信方式 + 鉴权码总表**（zrpc 走自研信封、HTTP 服务间用对称 bearer；kvstore 与 tokenizer、semantic 开发期免密）：

| 链路 | 协议 | 发起方携带的"码" | 接收方校验 |
|---|---|---|---|
| backend → kvstore | RESP | 无（开发期免密） | kvstore 未设 requirepass |
| backend → service | **zrpc** | zrpc REQUEST 信封的 `auth` 字段 = `Bearer me256487ang1chubdpdialoud22sev1ozhoguumyqca` | **C 层** `zrpc_server.c:233-243` 与 `accessToken` 常量时间比较 |
| service → sensitive / keyword | **zrpc** | zrpc 信封 `auth` = `Bearer ang1chubdev1ozhome256487d22sapguuv1ozhom` | 同上（filter 侧 `accessToken`） |
| service → tokenizer | HTTP | 无鉴权 | 内网地址可达即用（计 token，`/tokenizer/<model>`） |
| service → semantic | HTTP | 无鉴权（`dependOn.semantic` 只有 Address 字段），超时 1500ms | 内网地址可达即用（`/embed` 一次编码 + `/v1/decision[/batch]` 纯规则决策） |
| service → proxy | HTTP | `Authorization: Bearer i0jey84SdkFdw5u43780yjr3h7se8nth0yi295nr94ksDngKprEh` | proxy `http.access_token`（整体字符串相等） |
| service → kvstore | RESP | 无（开发期免密） | kvstore 未设 requirepass |
| proxy → DeepSeek | HTTP | `Bearer <真 key>`（`rand.Intn(len(api_keys))` 随机取一把） | DeepSeek 官方 |
| 浏览器 → backend | HTTP | `Authorization: <session token>`（裸 uuid，无 Bearer） | `AuthMiddleware` 查 kvstore `session:<token>` |

**要点**：
- **服务间 zrpc 鉴权不在 Go 代码里，而在 C 库**：客户端把 token 塞进 REQUEST 信封的 `auth` 字段（`zrpc_client.c:228` 拼 `"Bearer <token>"`），服务端 `authenticate()`（`zrpc_server.c:233-243`）**先尝试剥掉带空格的 `"Bearer "` 前缀**、再用**常量时间比较** `ct_eq` 比对 `access_token`；`access_token` 为 NULL 则完全关闭鉴权。**唯一免鉴权的是 PING/PONG 帧**（帧类型 7/8，`zrpc_server.c:301-316` 在方法查找前就回了）——**不是** gRPC Health 服务。
- ⚠ **旧描述里的"拦截器取 `metadata["authorization"]`"已不成立**：`ai-chat-service/interceptor/auth.go` 是 `grpc.UnaryServerInterceptor` 的死代码（无任何注册/调用），且 `keywords-filter/filter-server/interceptor/` **目录已不存在**。
...
sequenceDiagram
    ...
    participant SVC as ai-chat-service (zrpc :50055)
    participant SEM as semantic :3003
    participant SEN as sensitive (zrpc :50053)
    participant KW as keyword (zrpc :50054)
    ...
    KV-->>BE: device_id
    Note over BE,MYSQL: ④ 额度预检已移除（2026-09）：不再查 users.quota、不再回 402；token 只统计不扣减
    BE->>SVC: ⑤ zrpc chat.completion_stream (Id=uuid, Message, Pid)
    SVC->>SEN: ⑥ filter.validate(消息)
    alt 敏感词命中
        ...
        alt 语义缓存命中
            ...
            SVC->>KW: ⑫ filter.find_all 提取关键词（仅用于落库）
            ...
            SVC-->>BE: ⑱ zrpc 逐块转发 source=llm
        end
    end
    BE-->>FE: ⑲ NDJSON 逐行 Flush（打字机效果；**中间帧只带 delta，本段全文与 tokensUsed 只在末帧**）
    Note over BE,MYSQL: ====== 聊天后异步收尾 ======
    BE->>MYSQL: ⑳ EOF：只统计不分派——source=llm 记 tokensUsed、source=cache 记 tokensSaved（**不再 DeductQuota**）
...
- **backend 网关收口**：`ai-chat-backend/cmd/main.go:50-73`（路由注册，含 `/api/chat-process` 的 `RateLimit + AuthMiddleware`）+ `pkg/controllers/chat.go:69`（`ChatProcess`：鉴权取 device_id → **无额度预检** → zrpc 流式 → NDJSON 写出）。
- **backend 侧 zrpc 调用**：`ai-chat-backend/services/ai-chat-service/chat_stream.go:30-34`（`cli.Stream(ctx, contract.MethodChatCompletionStream, …)`）+ `:51-61`（逐包 `Recv`）；探活用 `services/ai-chat-service/ping.go`。
- **service 编排入口**：`ai-chat-service/chat-server/server/server.go:137`（`chatCompletionStream`，服务端流）；非流式 unary 在同文件 `:72`。
- **zrpc 服务端注册**：`chat-server/server/zrpc.go:27-37`（unary `chat.completion` + stream `chat.completion_stream`）。
- **上下文构建/裁剪**：`chat-server/server/app.go:134-208`（`buildChatCompletionRequest` / `rebuildMessages` / 裁剪循环）、`app.go:275-292`（`getContext` 沿 PID 链）。**token 预算裁剪算法详解见 [3.3](#33-token-预算裁剪rebuildmessages)**。
- **敏感词/关键词**：`app.go:302-326`（sensitive 失败 **fail-closed**；keywords 失败 **fail-open** 返回空表）。
- **语义缓存**：`chat-server/semcache/semcache.go`（`CacheQuery` / `CacheWrite` / `decisionOf` / `decisionBatch` / `backfillVec`）。
- **多轮上下文存储**：`chat-server/chat-context/redis.go`。

### 3.3 token 预算裁剪（rebuildMessages）

位置：`ai-chat-service/chat-server/server/app.go` `rebuildMessages()`（主体 `app.go:134-165`，裁剪循环 `:166-208`）；核心常量 `ChatPrimedTokens = 2`。配置：`max_tokens=8192`（2026-09 由 4096 上调，给 deepseek-v4-flash 推理留空间）、`min_response_tokens=2048`、`context_len=4`（`dev.config.yaml:17-38`，backend 侧同名字段见 `ai-chat-backend/dev.config.yaml:11-32`）。
...
**算法步骤**：
1. 若配置了 `bot_desc`，先数系统词的 token 数 `botTokens`（`app.go:134-165`）。
2. 数当前用户消息 `currTokens`；若 `currTokens > MaxTokens - MinResponseTokens - botTokens - ChatPrimedTokens` → 直接报错「请求消息超限」（单条就超预算，拒绝）`app.go:186-190`。
3. `tokens = currTokens + botTokens + ChatPrimedTokens`。
4. 沿 PID 链取历史上下文 `contextList`（`getContext`，最多 `context_len=4` 条，`app.go:275-292`），逐条尝试拼装：`if tokens + item.Tokens + ChatPrimedTokens > MaxTokens - MinResponseTokens { break }` —— **放不下就丢，从最早的历史开始丢**（裁剪循环 `app.go:166-208`）。
5. 拼完后把 `messages` 反转成"旧→新"顺序、再在头部插回 system 消息（同在 `app.go:134-208`）。
6. 返回的 `tokens` 作为请求总预算，`req.MaxTokens = 8192 - tokens`（`app.go:163`）交给上游。
...
> 文档差异提示：仓库早期文档里的「Redis 127.0.0.1:6379」是旧文案，**实际配置 `ai-chat-service/dev.config.yaml:39-42` 是 `host=127.0.0.1, port=5160, pwd:""`（免密直连）**，以配置为准。
...
**命中后从 kvstore 取，不是 MySQL**：MySQL 只存 `users`（用户记录，`quota` 字段仍在但**只作展示**）与 `chat_records`（历史）；语义缓存走 kvstore 快读（"秒回且免费"——注意**"免费"现在只是比喻**，额度扣减已移除，命中缓存省的是"一次 LLM 调用的时间与 operator 的 key 花销"）。
...
flowchart TD
    ...
    GETA --> RET["返回 source=cache (server.go:176-187)\n流式直出 · 不调 LLM · 不烧 key · 记 tokensSaved"]
```
代价：1 次 /embed + 1 次 GET(semfp) + 1 次 /v1/decision + 1 次 GET(答案) —— 比调一次大模型便宜两个数量级，且不消耗 operator 的 key 额度（"免费"是相对调 LLM 说的；额度扣减本身已于 2026-09 移除，见 §5.3）。
...
| 用户记录（`quota` **仅展示，不再扣减**） | — | `users`(id, device_id, quota, created_at) |
...
> ⚠ **2026-09 起本节第 3 小节已变**：**额度（计费）扣减逻辑被移除**，现在只统计 token 供前端展示；登录与限流不变。详见下文 §5.3。（额度已移除）
...
3. **后端校验**：`AuthMiddleware`（`ai-chat-backend/pkg/middlewares/auth.go:15-34`）：`Auth.Enabled=false` 直接放行；否则 `token := c.GetHeader("Authorization")` → `kredis.Get(context.Background(), "session:"+token)`：命中 → `c.Set("device_id", deviceID)` 后 `c.Next()`（下游写 `chat_records`、读 user 记录都从 gin ctx 读 device_id）；`redis.Nil` → **401**「未登录或登录已过期」并 `AbortWithStatusJSON`；其它错 → 500「会话服务异常」。
...
> 易混点：本系统**浏览器→backend 会话层**用的是**裸 token（无 Bearer）**；而**服务间 zrpc、service→proxy** 是 `Authorization: Bearer <token>`（§2.3 通信总表 / 答疑⑨）。两者校验各自实现，`AuthMiddleware` 不做 `TrimPrefix("Bearer ")`——把 Bearer 头发来反而不认。
...
- **状态存哪**：令牌桶是**进程内变量**（`RateLimitMiddleware` 闭包持有 `limiter`，`rate_limit.go:9`），**不在 kvstore、不按 IP**、也无跨实例共享。单实例部署够用；多副本部署时每副本各一个桶（总量被放大成 N×10），要更严格需 Redis 集中限流——本项目未做，属已知边界。

### 5.3 计费 / 额度（**扣减逻辑已于 2026-09 移除**，现只统计 token）

> **现状（按代码）**：`ChatProcess` **不再做额度预检、也不再扣减**——`chat.go:81` 注释"不做额度校验…对话不限额度"、`chat.go:139` 注释"只统计用量用于前端展示，不再扣减额度（额度限制已移除）"。`users.DeductQuota`（`pkg/users/users.go:47`）**仍在但全仓库无任何调用方**，属死代码。

**现在只剩"统计 + 展示"**（不再触碰 MySQL 的 quota）：

- 流结束后用 tokenizer 数 `prompt+answer`，**只填进响应里的统计字段**：
  - `source=llm`（公有大模型回答）→ `tokensUsed = pt+rt`
  - `source=cache`（缓存命中）→ `tokensSaved = pt+rt`（省下的量，前端"总节省"用）
- **实时刷新**：LLM 流式时每 15 个 chunk 用 tokenizer 刷新一次 `tokensUsed` 随包下发（`chat.go:191-206`），前端"总消耗"实时更新；缓存命中一次算完。
- 前端顶栏仍显示"额度"数字，但它**不再随使用变化**（因为没有任何扣减路径）。

**字段与残留（"删逻辑、留字段"）**：

| 位置 | 内容 | 状态 |
|---|---|---|
| `pkg/users/users.go:13` | `User.Quota` | 保留（建表/查询仍读写） |
| `pkg/config/config.go:51` + `ai-chat-backend/dev.config.yaml:47` | `auth.init_quota: 100000` | 保留，仅作建用户时的初值 |
| `pkg/db/mysql/mysql.go:36` | 列 `quota INT NOT NULL DEFAULT 100000` | 保留 |
| `pkg/controllers/auth.go:38,63` | login / `/api/session` 响应里的 `quota` | 保留（前端展示用） |
| `pkg/users/users.go:47` | `DeductQuota`（`UPDATE ... GREATEST(0, quota-?)`） | **死代码**（无调用方） |

**手工改额度（纯演示，不影响任何功能）**——表字段仍是 `id / device_id / quota / created_at`，建用户时写 `InitQuota=100000`；表由 backend 启动 `InitUsersTable()` 自动建：

```bash
# 查
mysql -uroot -p123456 ai_chat -e "SELECT device_id, quota FROM users;"
# 改（改完需重新登录/刷新会话才会在前端顶栏看到新数字；不改变任何限流/计费行为）
...
- **唯一业务路由**：`POST /tokenizer/<model>`（`tokenizer/tokenizer.py:18`），请求体是**单条 message** `{role, content, name?}`，成功响应 `{"code":200,"num_tokens":<int>}`、异常 `{"code":500,"msg":...}`（`tokenizer.py:26-35`）。
- **模型白名单**（`tokenizer.py:9-15`）：`gpt-3.5-turbo` / `gpt-3.5-turbo-16k` / `gpt-4` / `gpt-4-32k` / `deepseek-chat` / `deepseek-v4-flash` / `deepseek-v4-pro`（线上用 `deepseek-v4-flash`）。每条消息按 `[im_start]{role/name}\n{content}[im_end]\n` 折 +4 token。
- ⚠ **"未知模型"有两种不同处理**（容易写错）：`tiktoken.encoding_for_model(model)` 失败会**回退 `cl100k_base`**（`tokenizer.py:44-47`）；但紧接着 `num_tokens_from_messages` 里 `model not in support_models` 会**直接 `raise NotImplementedError`**（`tokenizer.py:59-61`）→ 路由 catch 后返回 **`code:500`**。所以不在白名单的模型**不是回退计数、而是报错**。
- **编码表是运行时联网下载的**：仓库内**没有本地 BPE 文件**，`tiktoken` 首次使用会从 OpenAI 公共 blob 端点下载并缓存（`encoding_cache` 只在进程内缓存 encoding 对象）——**首次冷启动需联网**。
- **没有 `/healthz` / `/readyz`**：`start.sh` 对它只做 TCP 端口探活（`port_listening`），与 semantic（有 `/healthz`+`/readyz`+`/model-info`）不对称。
- **计数差异**：本地 tiktoken 对中文/代码的计数与 DeepSeek 自身不同，长回答下会偏高（"8192 上限被数成 1 万+"是正常计数差）。
...
- **/readyz**：readiness，模型已载 + warmup 通过 → ok（含 model-info 明细）；失败 → `{"status":"error"}`。**/healthz**：liveness，进程活着即 ok（不依赖模型）——容器/脚本探活用它（旧 Swarm healthcheck 已随 `ai-chat-stack/` 废弃，见 §8.1）。
- 框架：`nuxt`（Python Web 微框架），`--workers 2`；ORT 线程由 `SEMANTIC_INTRA_OP`(默认4)/`SEMANTIC_INTER_OP`(默认1) 控制，可独立扩容（部署方式见 §8.1）。
- ⚠ **模型文件不在仓库里**（`semantic/models/e5s-v1/` 被 gitignore：`model.onnx` + `tokenizer.json` + `MANIFEST.json`）：fresh clone 必须 **`bash semantic/tools/fetch_model.sh`**（从 GitHub Release `models-e5s-v1` 下 tar.gz + sha256 校验）；Docker 构建期用 `ARG MODEL_URL` 下载。**缺模型时服务仍能启动**（warmup 失败只记日志），但 `/embed` 会返回 500 → 语义缓存整体退化为 miss（不是崩溃）。
...
### 8.1 生产部署：`docker/compose.yaml` + `deploy/`（**`ai-chat-stack/` 已废弃**）

> **⚠ 先纠正历史描述**：早期文档把 `ai-chat-stack/`（Docker Swarm + Harbor 私有镜像）当"生产"。**它现在已不用于部署**，里面全是过期内容（`port: 50051`、`address: ai-chat-service:50051`、`model: gpt-3.5-turbo`、backend `port: 80`、`replicas: 2`、Harbor 镜像 `192.168.239.161:5000/2404/*`）——只作存档，**别照它配**。

**当前生产路径 = 两条**：

1. **容器化（`docker/`）**：`docker/compose.yaml` 定义 **10 个服务**（kvstore / tokenizer / semantic / sensitive / keyword / mock / proxy / ai-chat-service / ai-chat-backend，全部 `build` **从源码构建**，不再依赖私有镜像仓库）；只有 backend 映射宿主端口 `127.0.0.1:7080:7080`，余者在网络内互访（服务名即主机名：`service:50055`、`sensitive:50053`、`keyword:50054`、`proxy:8084/v1`、`tokenizer:3002`）。
   - 配置**不写进 compose**，而是由 `deploy/scripts/render-config.sh` 把 `docker/config/*.yaml.envsubst` 模板 + `.env` 渲染成实际配置（`docker/config/*.yaml` 是渲染产物，仓库里只有模板）。
   - **compose 里已无任何 healthcheck 段**（旧的 `grpc_health_probe` 已被替换/移除，commit `c469c9f`）；就绪判据由 `deploy/scripts/deploy-app.sh` 用 **`curl /api/readyz`** 承担。
2. **裸进程（本机）**：`./start.sh`（见 §8.2）；对外公网走 FRP（见 §8.4）。

**两者关系**：`docker/README.md` 明确"是两条线"——宿主机上直接 `./start.sh` 与 `ai-chat-stack/` 不受 docker 前缀影响。公网入口（Nginx + frps）与本地栈的组合见 §8.4。

### 8.2 本地一键启停

- `./start.sh`：**补编译 → 按 `lib.sh` SERVICES 顺序启动 9 个进程 → 起 frpc（可选）**。日志 `runtime/logs/<name>.log`，pid `runtime/pids/<name>.pid`（`lib.sh:8-9,35-36`）。脚本**拒绝 root/sudo 运行**（`start.sh:7-10`），`--rebuild` 走全量重建。

  **阶段一：补编译**
  - **libzrpc.a 增量重建**（`start.sh:24-47`）：检测 `third_party/zrpc`（C 源）与 `zrpc-go` 是否有比 `build/libzrpc.a` 更新的文件 → 更新则 `make -C third_party/zrpc`，并**强制重编 3 个依赖它的 Go 服务**（`start.sh:46`）。
  - 5 个 Go 二进制（`start.sh:50-56`）：ai-chat-backend、ai-chat-service、keywords-filter、mock-openai-api、openai-api-proxy。触发：`--rebuild` / 二进制缺失 / 有更新的 `*.go`。
  - 前端（`start.sh:75-89`）：`cd ai-chat-web && pnpm install && pnpm build-only` → 产物 `dist/` → `cp` 到 `ai-chat-backend/www/`。backend 用 `http.FileServer(http.Dir("www"))` + NoRoute 落 `index.html` 托管（`cmd/main.go:67-73`）。**前端不作为独立进程**（开发态才另起 vite，端口 1002）。
  - kvstore（`start.sh:91-105`）：`make` 出 C 二进制。
  - 编译失败即 `exit 1`（`start.sh:107-111`）；另有两条**告警**：`semantic/models/e5s-v1/` 模型文件缺失（`start.sh:113-125`）、`DEEPSEEK_API_KEY` 未设置（`start.sh:127-131`）。

  **阶段二：按 `lib.sh` SERVICES 顺序启动 9 个进程**（启动循环 `start.sh:205-210`）
  - 顺序即依赖序（被依赖者在前）：`kvstore → tokenizer → semantic → sensitive → keyword → mock → proxy → service → backend`。
  - 每个进程由 `start_one`（`start.sh:188-203`）执行：`cd $cwd && nohup $cmd > runtime/logs/<name>.log 2>&1 & echo $! > runtime/pids/<name>.pid`，随后**轮询端口**（约 30×0.5s），端口在听即算就绪；失败打印日志尾部；端口已被占用则跳过（`✔ already running`）。全部就绪输出 `✔ 9/9 服务就绪 → http://localhost:7080`。
  - ⚠ **tokenizer 没有 HTTP 健康端点**，所以它的"就绪"就是 **TCP 端口在听**（见 §6.1）；其余同理是端口探活，**不代表依赖全部可用**（真正的依赖探测看 backend `/api/readyz`）。

  **阶段三（可选）：frpc**（`ensure_frpc`，`start.sh:137-186`）
  - 有 `deploy/app/.env` 且 `bin/frpc` 存在才起，否则跳过（`return 2`）；用 `deploy/scripts/lib.sh` 渲染 `frpc.yaml`；探活看 frps 的 39000 是否 ESTABLISHED（`lib.sh:51-54`）。**frpc 不计入上面的 9/9 计数**（有它才是第 10 个进程）。

  **日志路径约定**：
  - 进程 **stdout/stderr** → `runtime/logs/<name>.log`（nohup 重定向，name 即 SERVICES 名：`backend.log`、`service.log`、`sensitive.log`…）；
  - 应用自写日志走配置 `logPath`（相对各自 cwd）→ 各服务目录下 `runtime/logs/app.log`（例如 service 的 LLM 流式错误/`semcache_` 决策行在 `ai-chat-service/runtime/logs/app.log`）。
- `./stop.sh`：**逆序停止 9 个服务 + frpc**（frpc 首先被停，`stop.sh:11-15`）；优先 PID 文件 `kill -TERM`，端口占用则 `fuser -k -TERM` 回退；等待端口释放最长 10s，仍在则 `fuser -k -KILL`（`stop.sh:79-91`）；最后清理陈旧 pid 文件。**不清理日志/临时文件**。
...
本项目的"对外公网"不是把整套服务搬到云上，而是 **FRP 内网穿透**：云端一台 2C2G 只做**入口转发**（frps + Nginx + Certbot），**完整的 AnswerMesh（含 semantic 语义检索/tokenizer/kvstore/MySQL/向量库）始终只跑在本地节点**。完整技术文档见 `docs/echo-chat-frp-nat-traversal.md`。
...
flowchart LR
    ...
    subgraph loc["本地应用节点（运行整套 AnswerMesh）"]
        FRPC
        BE
    end
```
...
- 本地：`sudo bash deploy/scripts/deploy-app.sh` —— 渲染配置/frpc → 起 AnswerMesh（docker compose 或 `start.sh`）→ 等 **`GET /api/readyz`** 就绪 → 起 frpc。
...
云端只跑 frps + nginx 两个容器（`deploy/edge/compose.yaml`）。nginx 用 `network_mode: host`（直接听 80/443），渲染产物 `deploy/edge/nginx/answermesh.conf`（模板 `.envsubst`，变量仅 `PUBLIC_DOMAIN / FRP_VHOST_HTTP_PORT`，见 `deploy/scripts/deploy-edge.sh:46`）里职责是：
- **80 端口**：只做两件事——`location /.well-known/acme-challenge/` 供 certbot webroot 验证 + 其余 `return 301 https://$host$request_uri` 整段跳 https（模板 `answermesh.conf.envsubst:5-23`）。首次无证书时先用 `answermesh.bootstrap.conf.envsubst`（只开 80 收验证，其余 503），certbot 申请成功再原子换全量 conf（`deploy-edge.sh:142-173` 两阶段）。
- **443 端口 = TLS 终止**：证书 `/etc/letsencrypt/live/<PUBLIC_DOMAIN>/fullchain.pem`；`location /api/` 与 `location /` 都 `proxy_pass http://127.0.0.1:${FRP_VHOST_HTTP_PORT}`（envsubst 后即 **39001**，`answermesh.conf.envsubst:49-83`）；`/api/` 额外挂 API 限流 `limit_req zone=answermesh_api rate=10r/s burst=30 nodelay`、`proxy_buffering off`（流式禁缓冲）+ `proxy_read/send_timeout 600s`，并加安全响应头；另有 `location = /edge-healthz` 云侧存活自检。
...
frps 在 39001 收到 Nginx 转来的 HTTP 请求后，按 **Host 头**（=`PUBLIC_DOMAIN`，与本地 frpc `customDomains` 精确匹配，`frpc.yaml.envsubst:27-34`）路由到隧道里注册的 `answermesh-web`（`type: http, localIP: 127.0.0.1, localPort: 7080`）。即：**443→39001 是 Nginx 的 HTTP 反向代理（不是 iptables/TCP 映射），39001→7080 是 FRP 基于 Host 的应用层路由**——中间隔着 frps 的 vhostHTTP 监听 + 一条本地 frpc 主动维持的常驻隧道。证书续期由 `deploy-edge.sh` 的 reload hook 处理（`deploy-edge.sh:194` 建议写入 `/etc/letsencrypt/renewal-hooks/deploy/`）。

---

## 9. 本地运行与验证

**一键启动**：`cd <AnswerMesh 工作副本> && ./start.sh`（脚本自己会编 kvstore / Go 服务 / 前端，**不需要先 `make kvstore`**；需 `DEEPSEEK_API_KEY` 或 proxy 配置里有真 key）。

**验证一次完整对话**：
1. 浏览器 `http://localhost:7080` → **自动免密登录**（首访自动生成 device_id，无登录框、无验证码）→ 直接提问。
2. 健康检查（**都用 HTTP，没有 `grpc_health_probe` 了**）：
   ```bash
   curl -s localhost:18081/healthz      # sensitive（filter 1）存活
   curl -s localhost:18082/healthz      # keyword（filter 2）存活
   curl -s localhost:8080/healthz       # ai-chat-service 存活（/readyz 需 zrpc 已 Serve）
   curl -s localhost:7080/api/health    # backend 存活（空 handler，只回 200）
   curl -s localhost:7080/api/readyz    # ★ 真正有意义的就绪判据：并行探 mysql/kvstore/tokenizer/service
   ```
   - ⚠ **端口别记错**：zrpc 业务口（50053/50054/50055）**不是 HTTP**，用 `curl` 打不出结果；filter/service 的 HTTP 健康口是 **18081 / 18082 / 8080**（service 的 8080 是**硬编码**）。
   - `/api/readyz` 的第 4 项就是 **`ai_chat_service.Ping`，走的正是 zrpc**（`pkg/controllers/readyz.go:45`）——所以 `readyz` 返回 200 就等于"zrpc 链路通 + mysql/kvstore/tokenizer 都可达"。
   - `ai-chat-service/grpc_health_probe-linux-amd64` 与 `ai-chat-service/Dockerfile:9-10` 里的同名二进制是**历史残留**，不要再用。
...
| 项 | 文档说法 | 当前代码事实 | 说明 |
|---|---|---|---|
| **服务间协议** | gRPC（`ChatCompletionStream` / `Validate` / `FindAll`） | **自研 C zrpc v2 单栈**：`chat.completion[_stream]` / `filter.validate` / `filter.find_all`；gRPC 已删 | 见文首 2026-09-13 更新说明与 [2.3](#23-9-个进程清单与通信--鉴权码) |
| **服务间鉴权** | "拦截器取 `metadata["authorization"]`"（`interceptor/auth.go`） | **在 C 层**：信封 `auth` 字段 + `ct_eq` 常量时间比较；`interceptor/auth.go` 是死代码、`keywords-filter/filter-server/interceptor/` 目录已不存在 | 见 §2.3 要点 |
| **健康检查** | `grpc_health_probe -addr=:50051` | filter **HTTP 18081/18082** `/healthz` `/readyz`；service **HTTP 8080** `/healthz` `/readyz` `/metrics`；backend `/api/health`+`/api/readyz`；compose 无 healthcheck | 见 §9；`grpc_health_probe` 二进制是残留 |
| **生产部署** | Docker Swarm（`ai-chat-stack/`）+ Harbor 镜像 | **`docker/compose.yaml`（build-from-source，10 服务）+ `deploy/`（FRP）**；`ai-chat-stack/`（50051/gpt-3.5-turbo/replicas:2）**已废弃** | 见 §8.1 |
| **迁移期端口** | 50063/50064 作 zrpc 双栈口 | **已不存在**（`dev.config.yaml` 无 `zrpcPort`，代码不读该字段）；服务实际只监听 50053/50054/50055 | 50063 仅见于 `cmd/benchf` 与旧 Dockerfile EXPOSE |
...
| 语义缓存写入 | 早期 HSET 二进制 record | 明文 `SET <问题>=<回答>`(数组) + `HSET semd:e5s:v1:<问题>`(hash 向量，e5 命名空间版本化) | Q-A 走数组可读；向量归 `semd:e5s:v1:`（见 §4.2） |
| 语义缓存阈值 | 早期 0.15 / 0.10（Phase-1 word-vec 门 0.35 / 0.25 已随 e5 迁移废弃） | **acceptance_threshold 0.6 / min_margin 0.0**（决策由纯规则 `hard_decide` 承担，threshold 仅极低兜底）；`threshold`/`rerank_threshold` 已标 Deprecated | 见 §4.2 |
| 登录 | 短信验证码 | **device_id 免密自动登录**（无验证码） | 见 §5.1 |
| users 表 | phone 列 | `device_id` 列（`InitUsersTable` 自动建） | 见 §5 |
| **计费/额度** | "按 source 分派"：llm 扣、cache 不扣 | **扣减逻辑已整体移除**：不再预检、不再 `DeductQuota`（死代码）；只统计 `tokensUsed`/`tokensSaved`，`quota` 字段保留仅供展示 | 见 §5.3 |
| **模型** | `gpt-3.5-turbo`（旧配置） | 线上 `deepseek-v4-flash`（可配 + 请求级可覆盖） | 见 §11.4/§11.6 |
| **品牌** | 只有前端标题与域名是 AnswerMesh，代码/资源名沿用旧名 | **已全线统一**：前端标题、PWA 名、compose 项目名、nginx conf 名、frpc 代理名、文档 | 2026-09-13 完成更名 |
| 进程数措辞 | §2 早期版本曾写"8 个业务进程 + kvstore"、未含 semantic | `lib.sh:12-22` SERVICES 实为 **9 条且已含 kvstore**（业务进程 8 个）；含 frpc 才是第 10 个 | sensitive/keyword 共用 1 个二进制 → 9 进程 / 8 个二进制 |
| 代码行号 | 本文多处 `start.sh:77-92` / `app.go:136-147` 等 | 已随改动偏移，本版已按当前代码更新到 `start.sh:188-210`、`app.go:134-208` 等 | 若再改动以代码为准 |
...
> 本节记录 ai-chat 从"早期基线"演进到"当前状态"的完整路径。**前面 1-10 章描述的是早期基线**，若干细节已被下文取代；本会话改动全部提交到 github.com/robotlover-1（AnswerMesh + pocket-kv）。
>
> **标题里的日期区间（08-31 ~ 09-03）是初版范围，后续变更按序追加**：2026-09-04 起的 zrpc 单栈/健康检查/部署路线见 **§11.10**；2026-09-13 的全文代码核对见 **§11.11**。

### 11.1 仓库拆分：AnswerMesh + pocket-kv

- 从 `9.1-kvstore` 单仓拆成两个独立 repo：**AnswerMesh**（ai 助手，Go/Python/Vue3）+ **pocket-kv**（kvstore 存储，C）。
- AnswerMesh 内 `kvstore/` 为 submodule → pocket-kv（与 kvstore 内部引用 NtyCo 同理）。改动以 github 为准，本地工作副本 = git clone。
...
- 前端：每条回答徽标「公有大模型 / 缓存命中」；会话级「总消耗 / 总节省」统计（当时的语义是"LLM 扣额度、缓存命中不扣、每次都计入节省"——⚠ **扣减逻辑已于 2026-09 移除**，现在这两个数字**只统计不扣减**，见 §5.3）。
...
- 标题/favicon → **AnswerMesh** + 🤖（2026-09-13 全线更名，见文首更新说明；`ai-chat-web/index.html:10`）；去掉"由零声教学AI助手生成"文案。
...
### 11.10 2026-09-04 ~ 09-13：zrpc 单栈、HTTP 健康检查、compose 取代 Swarm、frpc 一键启停

- **zrpc 迁移完成（2026-09-04）**：4 条 gRPC 调用（backend→chat-service、chat-service→keywords-filter、以及 service 的 unary 入口）全部换成自研 **C zrpc v2 + NtyCo + cgo bridge**，业务逻辑零改动；方法名为短点号（`chat.completion[_stream]` / `filter.validate` / `filter.find_all`，见 §2.3）。**gRPC 传输已从业务链路删除**，残留仅为：`grpc-go` 依赖（供 `cmd/benchf` 与 `.pb.go` 编译）、`.pb.go`（**降级为内部 DTO**）、`interceptor/auth.go` 与 `services/grpc-client/*`（死代码）、`ai-chat-service/Dockerfile:9-10` 拷贝的 `grpc_health_probe`。
- **健康检查全面改 HTTP**（commit `c469c9f`）：`docker/compose.yaml` 的 healthcheck 段整体移除，就绪判据交给 `deploy/scripts/deploy-app.sh` 的 `curl /api/readyz`；filter 的 **18081/18082**、service 的 **8080** 成为唯一可用探活方式（zrpc 业务口不是 HTTP）。
- **部署路线收敛**：`ai-chat-stack/`（Swarm + Harbor 私有镜像 + `50051` + `gpt-3.5-turbo`）**废弃**，改为 `docker/compose.yaml`（10 服务、build-from-source）+ `deploy/`（app=frpc、edge=frps+nginx）两条脚本化路径（见 §8.1 / §8.4）。
- **frpc 纳入一键启停**（commit `0bfd2f4`）：`./start.sh` / `./stop.sh` 现在管理 frpc（需 `deploy/app/.env` + `bin/frpc`，否则自动跳过，`start.sh:137-186`）。
- **额度移除 + 品牌更名**：`ChatProcess` 去掉额度预检与 `DeductQuota`，只统计 token（§5.3）；前端标题改 **AnswerMesh**（2026-09-13 代码/资源名同步更名为 `answermesh`）。
- **zrpc 量化对比（务必带限定语）**：迁移期做过单 unary 微基准（同进程双栈 gRPC 50053 vs zrpc 50063，并发 8 × 5000 次；过程记录未随仓库保留）：**QPS 33,631 vs 26,605（+26%）**、avg 230µs vs 297µs、p99 **689µs vs 1542µs**。⚠ 该数字是**单盒 dev、单一 unary、首轮快照**，原文档自己也声明"不作为'必然更快'的定论"——引用时不能丢掉这个前提。

### 11.11 文档核对（2026-09-13）

本文按当前代码逐节复核，改动集中在四类过时描述：**gRPC / grpc_health_probe / Docker Swarm / 额度计费**（更正清单见文首 2026-09-13 更新说明与 §10 差异表）。同时把因代码演进偏移的行号（`start.sh`、`app.go`、`chat.go`、`readyz.go`、`main.go` 等）更新到当前值。

**仍存在的代码侧残留**（不影响功能，但读代码时会遇到，已在 §8.1/§10 标注）：`grpc-go` 依赖、`*.pb.go`、`interceptor/auth.go`、`services/grpc-client/*`、配置字段 `ZrpcAddress`/`Transport`、`ai-chat-service/Dockerfile` 的 `grpc_health_probe`、整个 `ai-chat-stack/`、以及**含明文密码但已不参与链路的 `vectorDB` 配置段**（`ai-chat-service/dev.config.yaml:82` 等）。
...
**kvstore（C）**
- `src/main/kvstore.c` NOAUTH 门禁 / AUTH / CLIENT 子命令 / SETEX / VSEARCH 分派（行号见文档 1 §5.2、§6.6）
- `src/storage/kvs_vector.c` VSEARCH 实现（扫描 global_hash，prefix 参数化/白名单 `{semcache:, semd:e5s:v1:}`、缺省 `semcache:`，暴力余弦 top-k）
- `configs/kvstore-ai.conf`：`port=5160`、**requirepass 注释掉（本地免密）**

**zrpc（服务间，自研）**
- `third_party/zrpc/`：C 协议库（帧 = magic `0x5A52` + ver + type + request_id + len + crc32；见 `include/zrpc_protocol.h`）；server 用 NtyCo（`src/zrpc_server.c`）；产出 `build/libzrpc.a`
- `zrpc-go/`：cgo bridge（`client.go` Unary/Stream、`server.go` RegisterUnary/RegisterStream、`stream.go` 断连取消）；`contract/methods.go:6-11` 方法名常量
- 接入点：`ai-chat-backend/services/ai-chat-service/chat_stream.go`、`ai-chat-service/services/keywords-filter/zrpc.go`、`ai-chat-service/chat-server/server/zrpc.go`

**语义缓存 & 上下文（Go）**
- `ai-chat-service/chat-server/semcache/semcache.go`：CacheQuery / CacheWrite / decisionOf / decisionBatch / backfillVec / probeModelConsistency
- `chat-server/chat-context/redis.go` GET(上下文)；`chat_context.go` ChatMessage{Id,Pid}
- `pkg/db/redis/redis.go` go-redis 连 kvstore（Password=AUTH）

**编排**
- `chat-server/server/server.go:137` 流式主流程（`:72` unary）；`app.go:134-208` token 预算裁剪、`app.go:275-292` getContext、`app.go:302-326` 敏感词/关键词

**网关 & 认证（含 token 统计，无额度）**
- `ai-chat-backend/cmd/main.go:50-73` 路由；`pkg/controllers/chat.go:69` ChatProcess；`pkg/controllers/readyz.go` /api/readyz（4 项探测）；`pkg/controllers/auth.go`；`pkg/middlewares/auth.go`（裸 token → `session:`）；`pkg/middlewares/rate_limit.go`；`pkg/users/users.go`（`DeductQuota` 为死代码）

**其它服务**
- `tokenizer/tokenizer.py`：`POST /tokenizer/<model>`（白名单 7 个模型；**无 /healthz**）
- `semantic/semantic.py`：`/embed` + `/v1/decision[/batch]` + `/healthz` + `/readyz` + `/model-info`（e5 384 ONNX；`/rerank` deprecated）；`semantic/models.py` e5 封装（模型文件需 `tools/fetch_model.sh`）；`semantic/decision.py` 纯规则 `hard_decide`
- `openai-api-proxy/`：`routers/routers.go`（`/health` + `/v1/*`）、`middleware/auth.go`（换 key）、`proxy/proxy.go`（ReverseProxy）；上游 `https://api.deepseek.com/v1`
- `keywords-filter/filter-server/`：`server/zrpc.go` 注册 `filter.validate` / `filter.find_all`（一个二进制两个实例，端口 50053/50054）

**部署与脚本**
- `lib.sh:12-22` SERVICES（9 进程）/ `:26-31` frpc；`start.sh`（编译+启动+frpc）；`stop.sh`
- `docker/compose.yaml`（10 服务 **build-from-source**，无 healthcheck）+ `docker/config/*.envsubst`（渲染模板）
- `deploy/scripts/render-config.sh` / `deploy-app.sh`（app=frpc）/ `deploy-edge.sh`（edge=Nginx+frps）
- `monitoring/`（Prometheus :9090 抓 `127.0.0.1:8080`，Grafana :3000）
- `ai-chat-web/`（Vue3 + Vite；`pnpm build-only` → 拷进 `ai-chat-backend/www`；开发态 vite :1002）
- ⚠ 已废弃：`ai-chat-stack/`（Swarm）、`ai-chat-service/interceptor/`、`services/grpc-client/`

---

*文档生成于 2026-08-26。2026-08-30 补充语义缓存阈值/rerank/8 进程通信等；2026-09-03 大幅演进——仓库拆分为 AnswerMesh+pocket-kv、免密登录去验证码、明文 Q-A 语义缓存、DeepSeek 接入、source/tokens 统计、推理模型截断治理、语义检索误命中治理、科幻前端；2026-09-04 Phase2+3 —— 语义检索迁 multilingual-e5(384/semd:e5s:v1:)、decision 纯规则化 `/v1/decision[/batch]`、Go 单次编码/profile 对齐验收、acceptance 0.6 校准，详见 **[11. 优化历程](#11-优化历程本会话-2026-08-31--09-03)**。2026-09-08 按代码逐行复核后，就地增补 10 条评审答疑（标注"答疑①~⑩ · 以代码为准"，分落 §4.1 / §4.2 附录 / §5.1 / §5.2 / §7 / §8.4），含红黑树命中/不命中两例的真实指纹与 e5 余弦实测。9 个进程清单、端点与关键路径均经代码逐行核实。*

*（2026-09-13 全文复核）按当前代码逐节重核并修正四类过时描述：**① gRPC → zrpc v2 单栈**（架构图/时序图/鉴权表/进程表全部改写，方法名改为 `chat.completion[_stream]`/`filter.validate`/`filter.find_all`；标注 `grpc-go`/`.pb.go`/`interceptor`/`grpc_health_probe` 为残留）；**② 健康检查改 HTTP**（filter 18081/18082、service 8080、backend `/readyz`；compose 已无 healthcheck）；**③ 生产部署改 `docker/compose.yaml` + `deploy/`**（`ai-chat-stack/` Swarm 栈标废弃），`frpc` 纳入一键启停；**④ 额度扣减逻辑移除**（§5.3 重写：只统计 `tokensUsed`/`tokensSaved`，`quota` 仅展示；`DeductQuota` 为死代码）。另修正 tokenizer 未知模型/无健康端点、semantic 模型文件需 `fetch_model.sh`、关键词仅落库、`app.go`/`chat.go`/`start.sh`/`readyz.go` 等行号偏移，并新增 §11.10/§11.11 记录 09-04 起的变更与本次核对范围。*
